...
class FlusherTests(TransactionTestCase):
    ...
    def test_on_progress_accumulates_calibs_and_frames(self):
        ...
        progress.on_progress(MasterCalibCreated(ImageType.BIAS, "cam1", "1x1", None, "bias.fits"))
        progress.on_progress(MasterCalibCreated(ImageType.DARK, "cam1", "1x1", None, "dark.fits", exptime=599.6))
        ...
        # Rounded once here, at the single point that writes calibs entries, so the
        # server-rendered template and the JS poller can't disagree on a half-second
        # boundary by applying different rounding rules to the same raw float.
        self.assertEqual(progress.calibs[1]["exptime"], 600)
        self.assertEqual(
            progress.frames,
